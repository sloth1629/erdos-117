...
\title{Abelian Covers and Pairwise Noncommuting Sets in Groups:\\
       Structural Reductions, an Exact Small Value, and a Counterexample}
...
computer-assisted arguments, $h(5)=5$ and $h(6)=6$; and construct scalar symplectic groups $S(q,m)$ with
$\aG(S(q,m))=q^m+1$.  A complete calculation in $S(3,2)$ gives
$(\nuG,\aG)=(7,10)$, disproving the candidate formula considered here,
$h(n)=\max\{n,2^{\lfloor(n-1)/2\rfloor}+1\}$.  The two computer-assisted
values use the subgroup-cover bounds $f(5)=16$ and $f(6)=36$, followed by
complete Schur-cover/exterior-square enumerations.  The remaining structural
results and the counterexample have computation-independent proofs.
\end{abstract}
...
Lemmas~\ref{lem:h5-enumeration} and~\ref{lem:h6-enumeration}.  Separate symmetry-reduced exact searches
...
The manuscript does not determine $h(n)$ beyond $n=6$, does not determine
